...
for fx in os.listdir(folder):
    ...
    for face in faces: 
        ...
        if who == None or (who != None and who != names[int(out)]):
            ...
            print('===========Kết quả===============')
            print("Tên: " + str(Right))
            print("Dự đoán: " + str(names[int(out)]))
            sum += 1
            if Right == str(names[int(out)]): right += 1
            else: 
                right1.append(Right)
                wrong1.append(str(names[int(out)]))
            cv2.rectangle(frame, (x,y), (x+w,y+h), (0,255,0), 2)
            print('=================================\n\n')
            break

print("Tổng số:" + str(sum))
print("Đoán đúng: " + str(right))
print("\nNgười bị đoán sai:")
right2, counts = np.unique(right1, return_counts=True)
print(dict(zip(right2, counts)))
print("\nNgười xuất hiện nhiều:")
wrong2, counts = np.unique(wrong1, return_counts=True)
print(dict(zip(wrong2, counts)))
